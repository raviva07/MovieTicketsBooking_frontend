  ...
  return (
    <div className="container my-4">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
          <Link to="/movies" className="btn btn-outline-secondary btn-sm">All Movies</Link>
        </div>
      </div>
      ...
      <div className="movie-grid d-flex flex-wrap gap-3" style={{ rowGap: 16 }}>
        {Array.isArray(movies) && movies.length > 0 ? (
          movies.map((m) => (
            <div key={m.id} className="d-inline-block text-center">
              ...
              <div className="mt-2">
                <Link to={`/shows?movieId=${m.id}`} className="btn btn-sm btn-outline-primary me-2">
                  View Shows
                </Link>
                <Link to={`/movies/${m.id}`} className="btn btn-sm btn-outline-secondary">
                  Details
                </Link>
              </div>
            </div>
          ...
          !loading && <p className="text-muted">No movies available.</p>
        )}
      </div>
    </div>
  );
